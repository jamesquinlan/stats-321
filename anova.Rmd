...
Let's check for outliers first since if present, we can remove them before checking the other assumptions.  Statistical tests for outliers include: Grubb's test, Dixon's test (small samples $n \le 25$), and Rosner's test (detect multiple outliers).


#### Grubb's Test

We can use `grubbs.test()` function from the Outliers package to test if the single higest value is an outlier [1].  The function takes several parameters (see `?grubbs.test`): 
...
* Fisher's F Statistic is used the criteria to reject the null hypothesis given several assumptions are satisfied.  F = MSR/MSE.
...
* To test these assumptions:
  ...
  4. Look at box and whisker's plot and run Grubb's test for outliers.  Remove them if present.  s
